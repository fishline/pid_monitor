    ...
    def main(args: Array[String]) {
                    ...
                    if (lines(i).contains(" METRICS ")) {
                        ...
                        if (metricsFields.length == 14) {
                            ...
                            if (metricsFields(5).contains("POOLNEWJVM") || metricsFields(5).contains("POOLREUSEJVM") || metricsFields(5).contains("FAILEDJVM")) {
                                ...
                                val sr = "^\\s+0+".r
                                pid = sr.replaceFirstIn(serverPidFields(0), "").toLong
                                ...
                                val cr = "^\\s+0+".r
                                pid = cr.replaceFirstIn(clientPidFields(0), "").toLong
                            }
        ...
        val pidIPTSTypeGC = dailyDataInput.filter{case (x, y) => x.contains("sdr_log")}.flatMap{ case (fn, fileContent) =>
            ...
            if (hi.hasNext) {
                ...
                var jvmRec = false
                var currentTS = 0
                var pidTypeTidNameMap = collection.mutable.Map[String, collection.mutable.Map[Long, String]]()
                                                        ...
                                                        if (tidM.hasNext) {
                                                            ...
                                                            if (tidName != "") {
                                                                if (!pidTypeTidNameMap.contains(pid + ":" + wType)) {
                                                                    pidTypeTidNameMap(pid + ":" + wType) = collection.mutable.Map[Long, String]()
                                                                    pidTypeTidNameMap(pid + ":" + wType)(tid.toLong) = tidName
                                                                } else if (!pidTypeTidNameMap(pid + ":" + wType).contains(tid.toLong)) {
                                                                    pidTypeTidNameMap(pid + ":" + wType)(tid.toLong) = tidName
                                                                }
                                                            }
                                                            if (pidTypeTidNameMap.contains(pid + ":" + wType) && pidTypeTidNameMap(pid + ":" + wType).contains(tid.toLong) && pidTypeTidNameMap(pid + ":" + wType)(tid.toLong).contains("ParallelGC")) {
        ...
        val finalTable = infoAllRed.map{ x =>
            ...
            for (i <- 0 until data2.length) {
                if (data2(i).contains("PID")) {
                    val tmpFields = data2(i).split(":")
                    pid = tmpFields(1).toString
                }
            }
            var wtype = ""
            for (i <- 0 until data2.length) {
            ...
            var speed = -1L
            if (rows.toLong > 10000 && duration_max > 5) {
                speed = rows.toLong / duration_max
            }
            
            cid + "," + appid + "," + hostip + "," + pid + "," + wtype + "," + startdate + "," + startts + "," + duration_ht + "," + duration_c + "," + rows + "," + gccost + "," + hottub + "," + errs + "," + mem + "," + duration_delta.toString + "," + duration_max.toString + "," + speed.toString
        }

        writer.write("ContainerID,ApplicationID,HostIP,PID,WType,StartDATE,StartTS,DuraHT,DuraC,Rows,GCCost,JVMType,Errors,MEM,DuraDelta,DuraMax,Speed\n")
