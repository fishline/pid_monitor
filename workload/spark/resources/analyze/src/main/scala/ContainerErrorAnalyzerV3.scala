...
object ContainerErrorAnalyzerV3 extends Logging {
    def main(args: Array[String]) {
        ...
        val containerTagMapB = sc.broadcast(containerTagMap)
            val failInfo1 = inputFail.map{ x =>
                val data = x._1.split(":")
                    containerTagMapB.value(data(0) + ":" + data(3)) + ":" + data(4) + "###NO_SUCH_BREAK###" + x._2
            }
        ...
        val cInfo = containerTSDuraPidWtypeHottub.map{ x =>
            ...
            val data1 = x._1.split(":")
                ...
                var value = new ArrayBuffer[(String, String)]
                val out = (data2(0) + ":" + data2(1), x._1)
                value += out
        ...
        failInfoAll.collect().foreach{ x =>
            var errors = "NA"
                for (i <- 0 until x._6.length) {
                    if (i == 0) {
                        errors = x._6(i).toString
                    } else {
                        errors = errors + "," + x._6(i)
                    }
                }
            writer.write(x._1 + ";" + x._2 + ";" + x._3 + ";" + x._4 + ";" + x._5 + ";" + x._6.length + ";" + errors + ";" + "\n")
        }
        writer.close()
