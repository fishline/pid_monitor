...
object CommonFriends extends Logging {
    ...
    def main(args: Array[String]) {
        ...
        val nIds = g.aggregateMessages[Buffer]( triplet => {
            triplet.sendToSrc(Buffer.create(triplet.dstId))
        }, (buf1, buf2) => buf1 ++= buf2)
        val oneNG = g.outerJoinVertices(nIds) { (vid, vdate, nOpt) =>
            nOpt match {
                case Some(nOpt) => {
        ...
        //scala.Predef..MODULE$.println(new scala.collection.mutable.StringBuilder().append("Original Graph Edge: ").append(scala.runtime.BoxesRunTime.boxToLong(org.apache.spark.graphx.Graph..MODULE$.graphToGraphOps(g, ClassTag..MODULE$.Int(), ClassTag..MODULE$.Int()).numEdges())).append("vertexCnt: ").append(scala.runtime.BoxesRunTime.boxToLong(org.apache.spark.graphx.Graph..MODULE$.graphToGraphOps(g, ClassTag..MODULE$.Int(), ClassTag..MODULE$.Int()).numVertices())).toString());
