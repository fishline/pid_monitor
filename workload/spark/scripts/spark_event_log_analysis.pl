...
chomp($master_ip);
my @event_log_fields = split(/\//, $event_log);
my $event_log_name = $event_log_fields[$#event_log_fields];
`$ENV{'SPARK_HOME'}/bin/spark-submit --master yarn --num-executors 8 --executor-cores 2 --executor-memory 4g --driver-memory 4g --class src.main.scala.EventLogParsing $ENV{'PMH'}/workload/spark/resources/analyze/target/scala-2.10/analyze_2.10-1.0.jar hdfs://$master_ip/tmp/pmh_spark_analyze/$event_log_name > /tmp/screen_output_$event_log_name 2>&1`;
my @info_fields = split(/\//, $info_fn);
my $log_path = $info_fn;
$log_path =~ s/info$/rundir/;
`echo $log_path >> /tmp/debug`;
`echo $info_fields[$#info_fields-5] >> /tmp/debug`;
`echo $log_path/$info_fields[$#info_fields-5]/latest/data/raw/analyze_$event_log_name.txt >> /tmp/debug`;
`cat /tmp/screen_output_$event_log_name | grep -v "INFO" | grep -v "WARN" | grep -v "===" | grep -v "^SLF4J" > $log_path/$info_fields[$#info_fields-5]/latest/data/raw/analyze_$event_log_name.txt`;
print "<a href=\"../data/raw/analyze_".$event_log_name.".txt\">Analyze result</a>\n";
exit 0;
