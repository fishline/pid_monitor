...
export WORKLOAD_NAME=spark_sql_vcore2_cms_dynamic-20170117214427
export DESCRIPTION="spark_sql_vcore2_cms_dynamic-20170117214427"
...
INFO=$PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/info
APPID=$PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/appid
DEBUG=$PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/debug.log
rm -f $INFO
...
do
    ...
    APP_ID_FROM_LOG=""
    grep "EventLoggingListener: Logging events to" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn1-ITER$ITER.log > /dev/null 2>&1
    if [ $? -eq 0 ]
    then
        APP_ID_FROM_LOG=`grep "EventLoggingListener: Logging events to" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn1-ITER$ITER.log | awk -F"file:" '{print $2}' | awk -F/ '{print $NF}'`;
    else
        grep "Submitted application" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn1-ITER$ITER.log > /dev/null 2>&1
        if [ $? -eq 0 ]
        then
            APP_ID_FROM_LOG=`grep "Submitted application" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn1-ITER$ITER.log | awk '{print $NF}'`;
        fi
    fi
...
do
    ...
    CMD="/home/hduser/spark-1.6.1-bin-hadoop2.3/bin/spark-shell --master yarn --executor-cores 2 --executor-memory 2g --driver-memory 4g --conf spark.executor.extraJavaOptions=\"-XX:MaxPermSize=256m -server -XX:+UseMembar -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:+CMSScavengeBeforeRemark -XX:ParallelCMSThreads=4 -XX:SurvivorRatio=4 -XX:+UseCMSCompactAtFullCollection -XX:+CMSClassUnloadingEnabled -XX:CMSInitiatingOccupancyFraction=70\" --conf spark.shuffle.service.enabled=true --conf spark.dynamicAllocation.enabled=true --conf spark.eventLog.enabled=true --conf spark.eventLog.dir=/tmp/sparkLogs --queue root.spark_queue.vcore2 --jars /home/maha/db-derby-10.11.1.1-bin/lib/derbyclient.jar -i /home/felix/Github/pid_monitor/workload/hive/test_case/spark-vcore3-iter1.scala"
    CMD="${CMD} > $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn2-ITER$ITER.log 2>&1"
    export WORKLOAD_CMD=${CMD}
    # Get existing application-id infos
    echo "FINISHED" > $APPID
    `$PMH/workload/spark/scripts/query_yarn_app_id_in_some_state.pl /home/maha/hadoop-2.2.0 FINISHED $DEBUG >> $APPID`;
    ...
    APP_ID_FROM_LOG=""
    grep "EventLoggingListener: Logging events to" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn2-ITER$ITER.log > /dev/null 2>&1
    if [ $? -eq 0 ]
    then
        APP_ID_FROM_LOG=`grep "EventLoggingListener: Logging events to" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn2-ITER$ITER.log | awk -F"file:" '{print $2}' | awk -F/ '{print $NF}'`;
    else
        grep "Submitted application" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn2-ITER$ITER.log > /dev/null 2>&1
        if [ $? -eq 0 ]
        then
            APP_ID_FROM_LOG=`grep "Submitted application" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn2-ITER$ITER.log | awk '{print $NF}'`;
        fi
    fi
...
do
    ...
    CMD="/home/hduser/spark-1.6.1-bin-hadoop2.3/bin/spark-shell --master yarn --executor-cores 2 --executor-memory 2g --driver-memory 4g --conf spark.executor.extraJavaOptions=\"-XX:MaxPermSize=256m -server -XX:+UseMembar -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:+CMSScavengeBeforeRemark -XX:ParallelCMSThreads=4 -XX:SurvivorRatio=4 -XX:+UseCMSCompactAtFullCollection -XX:+CMSClassUnloadingEnabled -XX:CMSInitiatingOccupancyFraction=70\" --conf spark.shuffle.service.enabled=true --conf spark.dynamicAllocation.enabled=true --conf spark.eventLog.enabled=true --conf spark.eventLog.dir=/tmp/sparkLogs --queue root.spark_queue.vcore2 --jars /home/maha/db-derby-10.11.1.1-bin/lib/derbyclient.jar -i /home/felix/Github/pid_monitor/workload/hive/test_case/spark-vcore3-iter8.scala"
    CMD="${CMD} > $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn3-ITER$ITER.log 2>&1"
    export WORKLOAD_CMD=${CMD}
    # Get existing application-id infos
    echo "FINISHED" > $APPID
    `$PMH/workload/spark/scripts/query_yarn_app_id_in_some_state.pl /home/maha/hadoop-2.2.0 FINISHED $DEBUG >> $APPID`;
    ...
    APP_ID_FROM_LOG=""
    grep "EventLoggingListener: Logging events to" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn3-ITER$ITER.log > /dev/null 2>&1
    if [ $? -eq 0 ]
    then
        APP_ID_FROM_LOG=`grep "EventLoggingListener: Logging events to" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn3-ITER$ITER.log | awk -F"file:" '{print $2}' | awk -F/ '{print $NF}'`;
    else
        grep "Submitted application" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn3-ITER$ITER.log > /dev/null 2>&1
        if [ $? -eq 0 ]
        then
            APP_ID_FROM_LOG=`grep "Submitted application" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn3-ITER$ITER.log | awk '{print $NF}'`;
        fi
    fi
...
do
    ...
    CMD="/home/hduser/spark-1.6.1-bin-hadoop2.3/bin/spark-shell --master yarn --executor-cores 2 --executor-memory 2g --driver-memory 4g --conf spark.executor.extraJavaOptions=\"-XX:MaxPermSize=256m -server -XX:+UseMembar -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:+CMSScavengeBeforeRemark -XX:ParallelCMSThreads=4 -XX:SurvivorRatio=4 -XX:+UseCMSCompactAtFullCollection -XX:+CMSClassUnloadingEnabled -XX:CMSInitiatingOccupancyFraction=70\" --conf spark.shuffle.service.enabled=true --conf spark.dynamicAllocation.enabled=true --conf spark.eventLog.enabled=true --conf spark.eventLog.dir=/tmp/sparkLogs --queue root.spark_queue.vcore2 --jars /home/maha/db-derby-10.11.1.1-bin/lib/derbyclient.jar -i /home/felix/Github/pid_monitor/workload/hive/test_case/spark-vcore3-iter2.scala"
    CMD="${CMD} > $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn4-ITER$ITER.log 2>&1"
    export WORKLOAD_CMD=${CMD}
    # Get existing application-id infos
    echo "FINISHED" > $APPID
    `$PMH/workload/spark/scripts/query_yarn_app_id_in_some_state.pl /home/maha/hadoop-2.2.0 FINISHED $DEBUG >> $APPID`;
    ...
    APP_ID_FROM_LOG=""
    grep "EventLoggingListener: Logging events to" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn4-ITER$ITER.log > /dev/null 2>&1
    if [ $? -eq 0 ]
    then
        APP_ID_FROM_LOG=`grep "EventLoggingListener: Logging events to" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn4-ITER$ITER.log | awk -F"file:" '{print $2}' | awk -F/ '{print $NF}'`;
    else
        grep "Submitted application" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn4-ITER$ITER.log > /dev/null 2>&1
        if [ $? -eq 0 ]
        then
            APP_ID_FROM_LOG=`grep "Submitted application" $PMH/workload/spark/test_case/spark_sql_vcore2_cms_dynamic-20170117214427/spark-vcore2-cms-dyn4-ITER$ITER.log | awk '{print $NF}'`;
        fi
    fi
